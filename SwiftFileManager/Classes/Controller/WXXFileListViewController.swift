...
public class WXXFileListViewController: UIViewController {
    ...
    private func initControllerFirstData() {
        if (navigationController?.viewControllers.count)! <= 1 {
            self.navigationItem.leftBarButtonItem = UIBarButtonItem.init(title: "cancel", style: .plain, target: self, action: #selector(navigationItemLeftClick))
        }else{
            self.navigationItem.rightBarButtonItem = UIBarButtonItem.init(title: "edit", style: .plain, target: self, action: #selector(navigationItemRightClick))
        }
    }
    //MARK: Action
    ...
    @objc private func navigationItemRightClick() {
        ...
        if isEdit {
            ...
            self.navigationItem.rightBarButtonItem = UIBarButtonItem.init(title: "complete", style: .plain, target: self, action: #selector(navigationItemRightClick))
        }else{
            self.navigationItem.rightBarButtonItem = UIBarButtonItem.init(title: "edit", style: .plain, target: self, action: #selector(navigationItemRightClick))
